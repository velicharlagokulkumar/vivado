...
void cli_init(void)
{
	...
	cliCmdTable = malloc (sizeof(cliCmds)/sizeof(cliCmds[0]) * sizeof(*cliCmdTable));

	cli_cpyCmds(cliCmds, cliCmdTable, 0, sizeof(cliCmds)/sizeof(cliCmds[0]));
	numCliCmds = sizeof(cliCmds)/sizeof(cliCmds[0]);
}
...
{
	CMDSTRUCT *tmpCmdTable;

	tmpCmdTable = realloc(cliCmdTable, (numCliCmds + numCmds) * sizeof(*tmpCmdTable));
	if(tmpCmdTable) {
		cli_cpyCmds(cmds2add, tmpCmdTable, numCliCmds, numCmds);

		cliCmdTable = tmpCmdTable;
		numCliCmds += numCmds;

	} else {
		xil_printf("Adding commands failed\r\n");
	}
}
...
/**
*
* cli_cpCmds copy commands to command table
*
* @param	TBD
*
* @return	TBD
...
*
******************************************************************************/
void cli_cpyCmds(CMDSTRUCT *cmds2add, CMDSTRUCT *cmdsTable, int curCmds, int numCmds)
{
	int i;
	int j=0;

	for(i=curCmds; i < curCmds+numCmds; i++) {
		my_strcpy(cmdsTable[i].cmdString, cmds2add[j].cmdString);
		my_strcpy(cmdsTable[i].cmdHelp, cmds2add[j].cmdHelp);
//		strlcpy(cmdsTable[i].cmdString, cmds2add[j].cmdString, CMD_STRING_LEN);
//		strlcpy(cmdsTable[i].cmdHelp,   cmds2add[j].cmdHelp,   CMD_HELP_LEN);
		cmdsTable[i].valCnt = cmds2add[j].valCnt;
		cmdsTable[i].cmdFunc = cmds2add[j].cmdFunc;
		j++;
	}
}
...
int cmdParseAndRun( void )
{
    ...
    found=0;
	if( cmdPtr != NULL ) {
		for(int i=0; i < numCliCmds; i++) {
			if( !strcmp(cmdPtr, cliCmdTable[i].cmdString) ) {
				func = cliCmdTable[i].cmdFunc;
				found = 1;
				if(numCmdVals == cliCmdTable[i].valCnt) {
					xil_printf("\n\r");
					(*func)(cmdVals);
					return XST_SUCCESS;
				} else {
					xil_printf("\n\rError: wrong number of values\n\r");
					return XST_FAILURE;
				}
			}
		}
		if(!found) {
			xil_printf("\n\rError: %s command not found\n\r", cmdPtr);
			return XST_FAILURE;
		}
	}
...
void doHelp(void) {
	...
	for(i=0; i<numCliCmds; i++) {
             xil_printf("    %s %s\r\n", cliCmdTable[i].cmdString, cliCmdTable[i].cmdHelp);
        }
}
