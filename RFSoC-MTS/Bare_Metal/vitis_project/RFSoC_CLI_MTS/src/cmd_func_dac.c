...
*
* @param	cmdVals[0] tile, cmdVals[1] dac
*
* @return	None
*
* @note		TBD
...
/*
	//Get DAC output current setting
	Status = XRFdc_GetOutputCurr(RFdcInstPtr, Tile_Id, Block_Id, &OutputCurr);
	if (Status != XST_SUCCESS) {
		xil_printf("XRFdc_GetOutputCurr() failed\r\n");
		return;
	}
	switch(OutputCurr) {
	case XRFDC_OUTPUT_CURRENT_20MA:
		xil_printf("DAC output current is 20mA\r\n");
		xil_printf("DAC_AVTT should be set to 2.5V\r\n");
		break;
	case XRFDC_OUTPUT_CURRENT_32MA:
		xil_printf("DAC output current is 32mA\r\n");
		xil_printf("DAC_AVTT should be set to 3.0V\r\n");
		break;
	default:
		xil_printf("DAC output current is not recognized\r\n");
		break;
	}
...
*/
//	xil_printf("\r\n###############################################\r\n");
//	xil_printf("=== DAC CUrrent test ===\n\r");


  //  xil_printf("DAC Current Status\r\n");
	for ( Tile_Id=0; Tile_Id<=1; Tile_Id++) {
		...
		switch(OutputCurr) {
		case XRFDC_OUTPUT_CURRENT_20MA:
			xil_printf("   DAC Tile%d Ch%d output current is 20mA.",Tile_Id,Block_Id);
			xil_printf("  DAC_AVTT should be 2.5V\r\n");
			break;
		case XRFDC_OUTPUT_CURRENT_32MA:
			xil_printf("   DAC Tile%d Ch%d output current is 32mA\r\n",Tile_Id,Block_Id);
			xil_printf("      -DAC_AVTT should be 3.0V\r\n");
			break;
		default:
